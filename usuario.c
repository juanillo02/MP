			...
			for(j = 0; j < (*nVehiculos); j++) 				//Recorre la estructura vehiculos
			{
																		...
																		//introducida anteriormente
				{
					...
					*vViajes=realloc((viajes *)(*vViajes),((*nViajes)+1)*sizeof(viajes));    //Se amplia el vector
					...
					fflush(stdin);
					fecha = comprobar_fecha((*vViajes)[* nViajes].F_inic, (*vViajes)[* nViajes].H_inic, (*vViajes)[* nViajes].H_fin);		//fecha llama a la funcion comprobar_fecha 
																																			//y si la fecha introducida es correcta la variable cambia a 1	
				}
